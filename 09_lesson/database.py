...
class Database:
    def __init__(self, connection_string):
        ...
        self.scripts = {
            "clean_test_users":
                "DELETE FROM users WHERE user_email LIKE '%@example.com'",
            "insert_user":
                "INSERT INTO users(user_email, subject_id) "
                "VALUES (:email, :subject_id)",
            "select_user_by_email":
                "SELECT * FROM users WHERE user_email = :email",
            "update_user_subject_by_email":
                "UPDATE users SET subject_id = :subject_id "
